<?php
/**
 * Peruse Featured Post Widget.
 *
 * @since 1.0.0
 */
if (!class_exists('Peruse_Featured_Post')) :
     ...
     * @since 1.0.0
     */
    class Peruse_Featured_Post extends WP_Widget
    {
        private function defaults()
        {
            $defaults = array(
                'title'    => esc_html__( 'Recent Posts', 'peruse' ),
                'cat'     => 0,
                'post-number'=> 5,
        ...
        {
            $opts = array(
                'classname' => 'peruse-featured-post',
                'description' => esc_html__('Displays Featured Post in Sidebars. Place it in Sidebar Widget Area.', 'peruse'),
            );

            parent::__construct('peruse-featured-post', esc_html__('Peruse Featured Post', 'peruse'), $opts);
        }
                            ...
                            if (has_post_thumbnail()) {
                                ?>
                                    <a href="<?php the_permalink(); ?>">
                                        <?php the_post_thumbnail('peruse-related-size'); ?>
        ...
        public function form($instance)
        {
            ...
            <p>
                <label
                for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'peruse'); ?></label>
            ...
            <p class="custom-dropdown-posts">
                <label for="<?php echo esc_attr($this->get_field_name('cat')); ?>">
                    <strong><?php esc_html_e('Select Category: ', 'peruse'); ?></strong>
                </label>
                <?php
                $cat_args = array(
                    'orderby' => 'name',
                    ...
                    'show_option_all' => esc_html__('Show Recent Posts', 'peruse')
                );
